...
VSCode extension は startup 時に `.ai-dev-os/session-boundary.json` を読み取り、bounded state を復元します。復元対象は pending rollover、stale session warning、compact continuity availability、session generation mismatch の確認に必要な summary-only state だけです。
